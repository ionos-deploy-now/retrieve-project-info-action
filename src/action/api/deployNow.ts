...
export default class DeployNowApi {
    ...
    getProject(projectId: string): Promise<Project> {
        return this.instance
                   .get(`/v3/accounts/me/projects/${projectId}`)
                   .then(res => res.data as Project)
    }

    findBranch(projectId: string, branchName: string): Promise<BranchOverview> {
        return new Retryable<BranchOverview>(async (retry, lastRetry) => await this.instance
                                                                           .get(`/v3/accounts/me/projects/${projectId}/branches?name=${branchName}`)
                                                                           .then(res => {
    ...
    getBranch(projectId: string, branchId: string): Promise<Branch> {
        return new Retryable<Branch>(async (retry, lastRetry) => await this.instance
                                                                           .get(`/v3/accounts/me/projects/${projectId}/branches/${branchId}`)
                                                                           .then(res => {
